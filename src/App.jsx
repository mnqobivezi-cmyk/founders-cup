...
function ChoirPage({role,user,local,setLocal,askPin,showToast}) {
  ...
  const load=useCallback(async()=>{
    try{
      const{data:ev}=await supabase.from("fc_events").select("id,choir_categories").eq("is_active",true).single();
      const eid=ev.id;
...
function ChoirManage({groups,scores,cats,published,askPin,showToast,onRefresh}) {
  ...
  const togglePublish=async()=>{const{data:ev}=await supabase.from("fc_events").select("id").eq("is_active",true).single();await supabase.from("fc_publish_flags").update({published:!published}).eq("event_id",ev.id).eq("competition","choir");showToast(published?"Hidden.":"Published!");onRefresh();};
  const addCat=async()=>{if(!newCat.trim())return;const{data:ev}=await supabase.from("fc_events").select("id,choir_categories").eq("is_active",true).single();const updated=[...(ev.choir_categories||cats),newCat];await supabase.from("fc_events").update({choir_categories:updated}).eq("id",ev.id);showToast("Category added!");setNewCat("");onRefresh();};
  const removeCat=cat=>askPin("Remove Category","Enter organizer PIN.",async()=>{const{data:ev}=await supabase.from("fc_events").select("id,choir_categories").eq("is_active",true).single();await supabase.from("fc_events").update({choir_categories:(ev.choir_categories||cats).filter(c=>c!==cat)}).eq("id",ev.id);showToast("Removed.");onRefresh();});
  return (
    <div className="pw">
